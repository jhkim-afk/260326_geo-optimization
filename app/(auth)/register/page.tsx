  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setServerError(json.error ?? '회원가입에 실패했습니다.');
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription>무료로 시작하세요 — 신용카드 불필요</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div>
              <Input placeholder="이름" {...register('name')} disabled={isSubmitting} />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name.message}</p>}
            </div>
            <div>
              <Input type="email" placeholder="이메일" {...register('email')} disabled={isSubmitting} />
              {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email.message}</p>}
            </div>
            <div>
              <Input type="password" placeholder="비밀번호 (대문자+숫자 포함, 8자 이상)" {...register('password')} disabled={isSubmitting} />
